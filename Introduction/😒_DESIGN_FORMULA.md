...
**致命限制**（任一满足即不适用）：
1. **密集采样**：$\rho \ge 0.3$ → 改用 EI/UCB（探索阶段已结束）
2. **极高维**：$d > 12$ → 交互组合爆炸，考虑降维或分组
3. **验证性研究**：效应已知仅需估计 → 用 D-optimal 设计
4. **单一目标**：仅需预测/仅需效应 → 用专用方法更优

**警告场景**（可用但次优）：
- **纯连续变量** + 充足预算：传统高斯过程方法可能更成熟
- **强先验知识**：已知主效应，用贝叶斯优化直接针对目标更高效
- **实时反馈**：每次采样耗时长（>10s），批量优势难发挥
- **跨被试泛化**：需大量被试数据，单被试采样策略效果有限

### 典型心理学场景评估

| 实验类型 | 适用性 | 理由 |
|---------|-------|------|
| Likert 量表 × 刺激因素 | ⭐⭐⭐⭐⭐ | 完美匹配（序数 + 混合变量 + 有限预算） |
| 反应时 × 实验条件 | ⭐⭐⭐⭐ | 适用（连续响应，但效应发现重要） |
| fMRI × 任务参数 | ⭐⭐⭐ | 可用（预算极限，需精简交互对） |
| 眼动追踪 × 界面设计 | ⭐⭐⭐⭐ | 适用（多指标，效应复杂） |
| 问卷调查 × 人口学变量 | ⭐⭐ | 次优（通常有理论假设，直接回归更好） |
