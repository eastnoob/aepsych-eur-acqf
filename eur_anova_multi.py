...
class EURAnovaMultiAcqf(AcquisitionFunction):
    ...
    def __init__(
        ...
        # ========== ARD权重（覆盖度加权）==========
        ard_weights: Optional[Union[str, Sequence[float]]] = None,  # Gower距离维度权重
        # ========== 局部扰动 ==========
        ...
        # ========== 解析ARD权重 ==========
        self.ard_weights = parse_ard_weights(ard_weights)
        ...
        if hasattr(model, "bounds"):
            ...
            if X_train.ndim >= 2:
                ...
                bounds = torch.tensor(
                    [[0.0] * num_dims, [1.0] * num_dims], dtype=torch.float32
                )
...
try:
    ...
    pass  # Config system may not be available in all contexts
