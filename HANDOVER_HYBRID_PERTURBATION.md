...
| 阶段 | 任务 | 状态 | 备注 |
|------|------|------|------|
| 阶段2 | Git准备 | ✅ | 创建 `feature/hybrid-perturbation` 分支 |
| 阶段3 | 代码修改 | ✅ | `LocalSampler`、`EURAnovaMultiAcqf` 已实现 |
| 阶段4.1 | 修改 local_num=6 | ✅ | 创建3个配置文件 |
| 阶段6（部分） | 文档更新 | ✅ | README.md、CHANGELOG.md 已更新 |

### 未完成部分（需继续）
| 阶段 | 任务 | 优先级 | 预估时间 | 备注 |
|------|------|--------|---------|------|
| **阶段1** | **序数模型验证** | 🔴 **最高** | 30分钟 | **必须先做！**采集函数可能降级 |
| 阶段5 | 对比测试 | 🟡 中 | 4-6小时 | 验证策略有效性 |
| 阶段6.2 | 单元测试 | 🟢 低 | 1小时 | 可选（如有测试框架） |
| 阶段6.4 | 合并到main | 🟡 中 | 15分钟 | 测试通过后执行 |
