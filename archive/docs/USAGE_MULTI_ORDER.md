...
acqf = EURAnovaMultiAcqf(
    ...
    tau1=0.7,        # r_t上阈值（>0.7时，模型不确定，降低交互权重）
    tau2=0.3,        # r_t下阈值（<0.3时，模型已收敛，提高交互权重）
    lambda_min=0.1,  # 最小交互权重（初期）
    lambda_max=1.0,  # 最大交互权重（后期）

    # 覆盖度权重
    gamma=0.3,
    use_dynamic_gamma=True,
    gamma_min=0.05,  # 最小覆盖权重（后期，样本充足）
    gamma_max=0.5,   # 最大覆盖权重（初期，样本稀少）
...
### Lambda (λ_t) - 交互效应权重的动态调整

**核心思想：课程学习（Curriculum Learning）- 由粗到细**
...
- **r_t ≈ 1.0**（高值）→ 初期，模型高度不确定，还在"懵懂状态"
- **r_t ≈ 0.0**（低值）→ 后期，模型已收敛，进入"精通状态"

#### 决策逻辑：

```python
if r_t > 0.7:  # 初期，模型懵懂
    λ_t = λ_min = 0.1  # 降低交互权重，聚焦主效应
    # 理由：数据稀疏时，连主效应都没搞清楚，
    #      强行关注交互会把噪声当成规律（过拟合）

elif r_t < 0.3:  # 后期，模型精通
    λ_t = λ_max = 1.0  # 提高交互权重，挖掘细节
    # 理由：主效应稳定后，为找到全局最优，
    #      必须捕捉微妙的非线性关系和交互效应
...
#### 潜在局限：
- ⚠️ **强交互系统**（如 f(x₁,x₂)=x₁·x₂，纯交互无主效应）可能初期误导
- ⚠️ **XOR类问题**（化学配方、组合爆炸场景）需特别注意

---

### Gamma (γ_t) - 覆盖度权重的动态调整

**核心思想：探索-利用平衡（Exploration-Exploitation）**
...
```python
if n < 3:     # 样本极少
    γ_base = γ_max = 0.5  # 高覆盖权重，广撒网

elif n ≥ 25:  # 样本充足
    γ_base = γ_min = 0.05  # 低覆盖权重，精细采样
...
```python
if r_t > 0.7:  # 模型懵懂（初期）
    γ_t = γ_base × 1.2  # 提高20%，加大探索力度

elif r_t < 0.3:  # 模型精通（后期）
    γ_t = γ_base × 0.8  # 降低20%，聚焦利用已知信息
...
#### 优势：
- ✅ 符合经典主动学习理论（UCB、Thompson Sampling）
- ✅ 初期广泛探索，避免陷入局部最优
- ✅ 后期精细利用，提高采样效率

#### 最终夹值：
```python
γ_t ∈ [0.05, 1.0]  # 硬限制，确保稳定性
```
...
样本数:   5                  15                30
─────────────────────────────────────────────────────
λ_t:      0.1 (聚焦主效应)   0.5 (平衡)        1.0 (挖掘交互)
γ_t:      0.5 (广撒网)       0.3 (渐收)        0.1 (精准打击)
策略:     探索主干          过渡阶段          精雕细节
```
...
**探索性研究**（不知道有什么交互）：
```python
use_dynamic_lambda = True   # 让系统自动调整
tau1 = 0.7
tau2 = 0.3
lambda_min = 0.1
lambda_max = 1.0
...
**验证性研究**（已知特定交互）：
```python
use_dynamic_lambda = False  # 固定权重
lambda_2 = 1.0              # 直接给高权重
```

**强交互系统**（化学配方、XOR问题）：
```python
use_dynamic_lambda = False  # 关闭课程学习
lambda_2 = 1.0              # 从一开始就关注交互
# 或者反转策略（需要修改代码）
```
