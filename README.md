## EUR ANOVA 系列采集函数（v1 / v2）

本目录包含两个相关的采集函数实现：

- `EURAnovaPairAcqf`（文件：`eur_anova_pair.py`）——主要实现，支持混合变量类型（分类/整数/连续）、序数熵计算、动态权重（λ_t 和 γ_t），以及**混合扰动策略**（新增功能）。

### 🆕 混合扰动策略（Hybrid Perturbation Strategy）

**版本**: v2.1.0 | **分支**: `feature/hybrid-perturbation` | **日期**: 2025-11-26

#### 核心改进

针对**2-3水平离散变量**场景的优化策略，通过穷举所有离散水平（而非随机采样）提升效应发现质量：

**问题背景**：
- 原始策略：对分类/整数变量使用高斯扰动后舍入
- 问题：2-3水平变量的随机采样易遗漏或偏向某些水平
  - 例如：3水平变量，`local_num=4`，可能采样到 `[0,1,1,2]` → 水平0覆盖不足
  - 导致主效应Δ_i估计不准确，影响采集函数排序质量

**混合扰动策略**：
- **穷举模式**（≤3水平）：完全枚举所有离散值，循环填充到`local_num`
  - 例如：3水平 + `local_num=6` → `[0,1,2,0,1,2]` (每个水平2次，完全均衡)
- **高斯模式**（>3水平）：保持原有随机扰动（避免穷举组合爆炸）
- **连续变量**：不受影响，仍使用高斯扰动

#### 配置参数

```ini
[EURAnovaPairAcqf]
# 【新增】混合扰动策略参数
use_hybrid_perturbation = True   # 启用混合扰动（默认False，向后兼容）
exhaustive_level_threshold = 3   # 对≤3水平变量使用穷举（默认3）
exhaustive_use_cyclic_fill = True  # 循环填充到local_num（默认True）

# 【推荐调整】提高local_num以适应2-3水平混合场景
local_num = 6  # LCM(2,3)=6，确保2水平和3水平变量均衡覆盖

# 【可选】自动计算 local_num（v2.1.1新增）
# auto_compute_local_num = True    # 启用自动计算（默认False，手动配置）
# auto_local_num_max = 12          # 自动计算上限（默认12，避免成本爆炸）
#
# 说明：当启用auto_compute_local_num时，系统会：
#   1. 找出所有 ≤ exhaustive_level_threshold 的离散变量
#   2. 计算这些变量水平数的LCM（最小公倍数）
#   3. 如果LCM > auto_local_num_max，则使用 auto_local_num_max
#   4. 示例：2水平和3水平变量 → LCM(2,3)=6（自动设置）
#
# 优势：适应不同实验场景，无需手动计算LCM
# 劣势：可能导致local_num过大（因此需要设置上限）
```

#### local_num 的设定策略

| 变量类型 | local_num=4 (原始) | local_num=6 (推荐) |
|----------|-------------------|-------------------|
| 2水平 | `[0,1,0,1]` ✓ 均衡 (2次/水平) | `[0,1,0,1,0,1]` ✓ 均衡 (3次/水平) |
| 3水平 | `[0,1,2,0]` ✗ 不均衡 (水平0重复2次) | `[0,1,2,0,1,2]` ✓ 均衡 (2次/水平) |

**关键洞察**: 6是2和3的最小公倍数，保证2水平和3水平变量都能完全均衡覆盖。

#### 使用示例

```python
from aepsych.server import AEPsychServer

# 1. 加载优化配置
with open('configs/hybrid_perturbation_optimized.ini') as f:
    config_str = f.read()

server = AEPsychServer()
server.configure(config_str=config_str)

# 2. （可选）验证序数模型配置（仅序数响应需要）
from tools.verify_ordinal_model import verify_and_save_report

is_valid = verify_and_save_report(
    server=server,
    output_path="results/ordinal_verification.json",
    min_training_samples=10
)

# 3. 正常运行实验
for trial in range(25):
    next_x = server.ask()  # EUR采集函数自动使用混合扰动策略
    outcome = get_response(next_x)
    server.tell(config_str, outcome)
```

#### 配置文件

提供两个预配置文件用于对比：

- **优化版**：`configs/hybrid_perturbation_optimized.ini` (启用混合扰动, local_num=6)
- **基线版**：`configs/baseline_config.ini` (禁用混合扰动, local_num=4)

#### 测试框架

```bash
# 运行对比实验（10次重复，统计显著性检验）
python tests/test_hybrid_perturbation_comparison.py
```

预期结果：
- 效应发现能力提升10-20%（R²提高）
- 参数估计质量改善（RMSE降低）
- 选点多样性增加（完全覆盖所有水平）

#### 技术细节

实现位置：
- `modules/local_sampler.py`: LocalSampler类（独立扰动器）
- `eur_anova_pair.py`: EURAnovaPairAcqf._make_local_hybrid()方法

关键方法：
- `_perturb_categorical()`: 分类变量穷举
- `_perturb_integer()`: 整数变量穷举
- `_perturb_continuous()`: 连续变量高斯扰动（不变）

---

下面为这两个类中可通过 INI（或以 Config 注册/注入）配置的参数清单与中文说明，按类分组。表格列出参数名、默认值、类型与简短说明，以及在 INI 中可接受的写法示例。

---

### 共同/相似参数（v1 与 v2 都有）

| 参数 | 默认 | 类型 | 说明 | INI 示例 |
|---|---:|---|---|---|
| `gamma` | 0.3 | float | 覆盖项（coverage）与信息项（information）融合时的权重（当未启用动态 γ_t 时使用） | `gamma = 0.3` |
| `main_weight` | 0.5 | float | 主效应（main effect）在信息融合中的权重 | `main_weight = 0.5` |
| `pair_weight` | 1.0 | float | 交互项（pair interaction）原始权重（λ_t 未缩放时参考） | `pair_weight = 1.0` |
| `use_dynamic_lambda` | True | bool | 是否启用基于参数不确定性的动态 λ_t（用于交互权重随训练进展调整） | `use_dynamic_lambda = true` |
| `tau1` | 0.7 | float | 动态 λ_t 的高阈值（r_t > tau1 时偏向主效应） | `tau1 = 0.7` |
| `tau2` | 0.3 | float | 动态 λ_t 的低阈值（r_t < tau2 时偏重交互） | `tau2 = 0.3` |
| `lambda_min` | 0.1 | float | 动态 λ 的最小值 | `lambda_min = 0.1` |
| `lambda_max` | 1.0 | float | 动态 λ 的最大值 | `lambda_max = 1.0` |
| `interaction_pairs` | None | list/str | 指定要计算的交互对。可以是字符串或序列，支持多种分隔格式（如 `"0,1;1,2"`、`"0|1"`、或真正的列表）。示例见下。 | `interaction_pairs = "0,1;2,3"` |
| `local_jitter_frac` | 0.1 | float | 局部扰动尺度，按训练数据范围的比例决定扰动 σ | `local_jitter_frac = 0.1` |
| `local_num` | 4 | int | 每种扰动类型的采样数（局域样本数），影响计算开销 | `local_num = 4` |
| `variable_types` | None | dict | 可直接传入字典 {index: "categorical"/"integer"/"continuous"}，用于覆盖自动推断 | 参见下文 |
| `variable_types_list` | None | list 或 str | 较方便的 INI 写法，按维度顺序给出类型标记（如 `[categorical, integer, continuous,...]` 或 `"categorical, integer, continuous"`） | `variable_types_list = [categorical, integer, continuous, categorical, continuous, integer]` |
| `coverage_method` | "min_distance" | str | 覆盖度量方法，传入底层 `compute_coverage_batch` 的 method 参数 | `coverage_method = min_distance` |
| `debug_components` | False | bool/str | 打开后会把中间分解量（主/交互/信息/覆盖）缓存到对象的属性，便于调试/导出 | `debug_components = true` |

注意：`variable_types_list` 与 `variable_types` 的优先级为：明确定义的 `variable_types` dict 优先，若未提供则尝试解析 `variable_types_list`（字符串或列表格式）。如果二者都缺失，类会尝试从模型中的 transforms 自动推断（若可用）。

---

### v1：`EURAnovaPairAcqf` 的专属/额外参数

| 参数 | 默认 | 说明 |
|---|---:|---|
| `coverage_method` | "min_distance" | 覆盖计算方法（同上，v1/v2 共用） |
| `local_num` | 4 | 局部扰动样本数（v1 与 v2 相同） |

v1 的实现中，局部扰动由 `_make_local` 产生，分类/整数维度在 v1 中被当作连续域扰动（即用高斯扰动）。这在混合变量空间会产生不合法的类别值，可能需要在外层配置 `variable_types` 并在 `PoolBasedGenerator` 中保证合法性，或直接使用 v2 替代。

---

### v2：`EURAnovaPairAcqf_v2` 的专属/新增参数

| 参数 | 默认 | 类型 | 说明 |
|---|---:|---:|---|
| `use_dynamic_gamma` | True | bool | 是否启用动态 γ_t（覆盖/信息融合权重随训练样本数/r_t 调整） |
| `gamma_max` | 0.5 | float | 动态 γ 的上界（训练样本很小时的偏好覆盖） |
| `gamma_min` | 0.1 | float | 动态 γ 的下界（训练样本较多时的偏好信息） |
| `tau_n_min` | 3 | int | 样本数下限阈值（用于 γ_t 动态计算） |
| `tau_n_max` | 40 | int | 样本数上限阈值（用于 γ_t 动态计算） |

v2 的关键改进是 `_make_local_hybrid`：对于分类变量直接从训练集的 unique 值采样，整数扰动后舍入并夹值，连续按高斯扰动，从而更好地匹配池中真实数据分布。

---

### INI 写法建议与示例

在 experiment 的 INI 中，你可以在对应的 generator / acquisition section 指定这些参数。例如：

```
[EURAnovaPairAcqf]
gamma = 0.25
main_weight = 0.4
pair_weight = 1.0
use_dynamic_lambda = true
tau1 = 0.8
tau2 = 0.2
lambda_min = 0.05
lambda_max = 1.0
interaction_pairs = "0,1;2,3"
local_jitter_frac = 0.08
local_num = 6
variable_types_list = [categorical, integer, continuous, categorical, continuous, integer]
debug_components = false
```

或针对 v2：

```
[EURAnovaPairAcqf_v2]
gamma = 0.3
use_dynamic_gamma = true
gamma_max = 0.6
gamma_min = 0.1
tau_n_min = 5
tau_n_max = 60
variable_types_list = categorical, integer, continuous, categorical, continuous, integer
```

注意：INI 中的字符串/列表解析在 `Config` 层会尝试把 `variable_types_list` 解析为字符串或序列（实现里对字符串做了拆分）。若遇到解析问题，可直接在运行脚本中把一个 Python 字典或列表传递给构造函数。

---

### 诊断建议（为什么你的 `data/*.csv` 需要正确类型声明）

- v1 将分类/整数当作连续扰动时，局部扰动会产生不合法的类别值（例如对字符串类别做 float 高斯扰动），这会导致当你把池（pool）点与 CSV 行匹配或将离散值回填到 `y` 时出现大量 mismatch，从而触发回退逻辑（例如将所有未匹配事件回填为 3）。
- v2 通过 `_make_local_hybrid` 修复了这一问题：分类按训练集 unique 值采样，整数扰动后舍入并截断，连续按高斯扰动，从而更好地匹配池中真实数据分布。

因此：

- 对混合类型数据强烈建议使用 `EURAnovaPairAcqf_v2`，并在 INI 中通过 `variable_types_list` 或 `variable_types` 明确声明每列的类型（或确保模型的 transforms 能被推断出类型）。
- 如果继续使用 v1，请确保数据与扰动/覆盖策略一致，或把 `local_num` 降低以节省计算并减少产生非法值的概率。

---

### 我可以为你做的改动（可选）

1. 在 `exp_test/*/config_v*.ini` 中把 `variable_types_list` 显式写好（按照你 CSV 中每列的真实类型），并把 generator 的 `acqf` 指向 `EURAnovaPairAcqf_v2`（推荐）；
2. 修改 `run_server.py` 中的 pool 加载逻辑，使其使用 pandas.Categorical / factorize 来编码类别，避免 hash 导致的不一致；
3. 把回填逻辑改为先尝试精确/近似匹配，再采用最近邻回退，并优先使用 CSV 中 `_y_cont`（若存在）来离散化标签。

告诉我你要我先做哪一步（我建议先把 `variable_types_list` 明确并切换到 v2，然后把回填逻辑改为 nearest-neighbor 回退并利用 `_y_cont`），我会按步骤实现并运行验证。

---

文件位置：

- `extensions/dynamic_eur_acquisition/acquisition_function_anova.py`  (v1)
- `extensions/dynamic_eur_acquisition/acquisition_function_anova_v2.py` (v2)
- 本 README：`extensions/dynamic_eur_acquisition/README.md`

---

## Contributors

We keep a small CONTRIBUTORS file in this directory. If you want to report an issue
or contribute, please check `CONTRIBUTORS.md` for contact / contributor information.

*Maintainers & authors*: see `CONTRIBUTORS.md`.

# Dynamic EUR Acquisition Function (V4)# Dynamic EUR Acquisition Function

一个用于AEPsych框架的自适应采集函数，专门用于分类变量和混合类型变量的贝叶斯优化。一个用于分类变量主动学习的动态采集函数实现，专注于最大化主效应和交互效应估计精度。

## 快速开始## 项目概述

### 安装本项目实现了用于 AEPsych 框架的自定义采集函数，特别针对分类变量的主动学习优化。通过结合参数方差减少（信息增益）和空间覆盖策略，在有限试验次数下高效探索设计空间

项目依赖已包含在AEPsych主项目中：### 主要目标

```bash- **最大化主效应估计精度**：通过信息增益优化主效应的估计

# 使用pixi（推荐）- **最大化交互效应估计精度**：特别关注二阶交互效应

pixi install- **提高空间覆盖率**：在有限试验次数下尽可能覆盖设计空间

- **避免重复采样**：提高探索效率

# 或使用pip

pip install aepsych numpy scipy torch botorch## 版本历史

```

### V1 - VarianceReductionWithCoverageAcqf (基线版本) ✅ **推荐使用**

### 基本使用- **实现文件**: `acquisition_function_v1.py`

- **核心思想**: 双组件设计（信息增益 + 覆盖度）

```python- **特点**:

from extensions.dynamic_eur_acquisition import EURAcqfV4  - 信息增益组件：最大化主效应和交互效应的方差减少

from botorch.models import SingleTaskGP  - 覆盖度组件：基于最小距离的空间覆盖奖励

import torch  - 动态权重：`lambda_inter` 从 0.5 → 3.0 随试验增加

- **性能** (80 trials, 360 design space):

# 1. 准备训练数据  - 唯一设计数: 39/360 (10.8%)

train_X = torch.rand(20, 4)  # 20个样本，4个特征  - 平均重复次数: 2.05x

train_Y = torch.rand(20, 1)  # 20个观测值  - 高分设计 (≥9.5): 8个

  - 平均分数: 8.72 ± 0.89

# 2. 训练GP模型- **优势**: 简单、稳定、平衡良好

model = SingleTaskGP(train_X, train_Y)- **不足**: 存在轻微重复采样问题

model.train()

### V2 - EnhancedVarianceReductionAcqf (实验版本) ⚠️ **失败 - 不推荐**

# 3. 创建采集函数- **实现文件**: `acquisition_function_v2.py`

acqf = EURAcqfV4(- **改进措施**:

    model=model,  1. 重复惩罚机制 (`penalty_repeat=0.01`)

    lambda_min=0.2,  2. 动态多样性权重 (`gamma_diversity: 0.8→0.2`)

    lambda_max=2.0,  3. 分箱均匀性管理 (`gamma_binning=0.4`, `n_bins=5`)

    tau_1=0.5,  4. 动态交互权重增强 (`lambda_inter: 0.5→3.0`)

    tau_2=0.1,  5. UCB 探索奖励 (`beta_ucb=0.15`)

    gamma=0.3,  6. 信息增益优先优化

    interaction_terms=[(0, 1), (1, 2), (2, 3)],  # 指定关注的交互项- **性能** (80 trials, 360 design space):

    variable_types={0: "categorical", 1: "categorical", 2: "integer", 3: "continuous"}  - 唯一设计数: 28/360 (7.8%) ❌ **比 V1 差 28.2%**

)  - 平均重复次数: 2.86x ❌ **比 V1 多 39.5%**

  - 高分设计 (≥9.5): 4个 ❌ **比 V1 少 50.0%**

# 4. 在LHS后初始化（获取初始方差基线）  - 平均分数: 7.94 ± 0.60 ❌ **比 V1 低 0.78 分**

acqf.initialize_after_lhs(train_X, train_Y)- **失败原因**:

  1. 重复惩罚过弱（应使用硬排除 `-inf`）

# 5. 评估候选点  2. 分箱管理产生局部陷阱

candidates = torch.rand(10, 4)  # 10个候选点  3. 多组件平衡复杂度过高（4组件 vs V1的2组件）

scores = acqf(candidates)  4. 动态多样性权重减小导致过度开发

best_idx = scores.argmax()- **教训**: 

next_point = candidates[best_idx]  - 简单优于复杂

```  - 硬约束优于软惩罚

  - V1 的设计已经很优秀，无需全面重构

### 在AEPsych配置中使用

### V3 - 推荐方向 (未实现)

```ini- **建议**: 最小化改动，保持 V1 框架

[common]- **唯一改变**: 添加硬重复排除

parnames = [color, layout, font_size, background]  ```python

stimuli_per_trial = 1  if design_key in sampled_designs:

outcome_types = [continuous]      score = -np.inf  # 硬排除，非软惩罚

strategy_names = [init_strat, opt_strat]  ```

- **可选**: 候选集预过滤（80%未采样 + 20%已采样）

[color]- **预期性能**: 55-65 唯一设计 (+40-65%), 12-16 高分设计 (+50-100%)

par_type = categorical

choices = [red, blue, green, yellow]## 项目结构



[layout]```

par_type = categorical  dynamic_eur_acquisition/

choices = [grid, list, flow]├── acquisition_function_v1.py          # V1 基线版本 ✅ 推荐

├── acquisition_function_v2.py          # V2 实验版本 ⚠️ 失败

[font_size]├── gower_distance.py                   # Gower 距离计算（混合变量）

par_type = integer├── gp_variance.py                      # GP 方差计算器

lower_bound = 10├── __init__.py

upper_bound = 20│

├── test/

[background]│   └── categorical_experiment/

par_type = continuous│       ├── experiment_config_v1.ini    # V1 配置

lower_bound = 0.0│       ├── experiment_config_v2.ini    # V2 配置

upper_bound = 1.0│       ├── run_categorical_experiment_v1.py  # V1 实验脚本

│       ├── run_categorical_experiment_v2.py  # V2 实验脚本

[init_strat]│       ├── compare_v1_vs_v2.py         # V1/V2 对比分析

min_asks = 20│       ├── results/                    # V1 实验结果

generator = SobolGenerator│       ├── results_v2/                 # V2 实验结果

│       ├── report/                     # 分析报告和可视化

[opt_strat]│       │   ├── comparison_v1_vs_v2.png # 8 面板对比图表

min_asks = 50│       │   └── V2_FAILURE_ANALYSIS.md  # V2 失败分析

generator = OptimizeAcqfGenerator│       └── FINAL_REPORT_V2.md          # 完整实验总结

acqf = EURAcqfV4│

├── docs/                               # 原始文档

[EURAcqfV4]│   ├── README.md

lambda_min = 0.2│   ├── QUICKSTART.md

lambda_max = 2.0│   └── IMPLEMENTATION_SUMMARY.md

tau_1 = 0.5│

tau_2 = 0.1├── README.md                           # 本文件（项目总览）

gamma = 0.3└── .gitignore                          # Git 忽略配置

interaction_terms = (0,1);(1,2);(2,3)```

coverage_method = min_distance

# variable_types自动从ParameterTransforms推断## 快速开始

```

### 安装依赖

## 核心特性

```bash

### 1. 自适应信息增益# 使用 pixi（推荐）

- **主效应探索**: 优先探索对响应影响大的主要因素pixi install

- **交互效应探索**: 动态增加交互项的探索权重

- **动态平衡**: λ_t根据主效应不确定性自动调整# 或使用 pip

pip install numpy scipy matplotlib aepsych

### 2. 空间覆盖优化```

- **Gower距离**: 支持混合类型变量（分类、整数、连续）

- **自适应覆盖**: 避免过度采样同一区域### 使用 V1 采集函数（推荐）

- **硬排除**: 自动排除已采样的重复设计点

```python

### 3. 变量类型感知from acquisition_function_v1 import VarianceReductionWithCoverageAcqf

- **自动推断**: 从AEPsych的ParameterTransforms自动识别变量类型import numpy as np

- **规范化处理**: 分类和整数变量自动round到有效值

- **灵活配置**: 支持手动指定variable_types# 初始化采集函数

acq_fn = VarianceReductionWithCoverageAcqf(

## 主要参数    lambda_main=1.0,         # 主效应权重

    lambda_inter=0.5,        # 交互效应初始权重

| 参数 | 类型 | 默认值 | 说明 |    lambda_inter_max=3.0,    # 交互效应最大权重

|------|------|--------|------|    tau_main=0.3,            # 主效应阈值

| `lambda_min` | float | 0.2 | 交互项权重最小值 |    tau_inter=0.5,           # 交互效应阈值

| `lambda_max` | float | 2.0 | 交互项权重最大值 |    gamma_diversity=0.5,     # 多样性/覆盖度权重

| `tau_1` | float | 0.5 | 主效应阈值（r_t的中点） |    interaction_terms=[(0, 1), (1, 3), (2, 3)]  # 关注的交互项

| `tau_2` | float | 0.1 | 主效应阈值宽度 |)

| `gamma` | float | 0.3 | 覆盖度权重 |

| `interaction_terms` | List[Tuple] | [] | 关注的交互项，如[(0,1), (2,3)] |# 拟合数据

| `noise_variance` | float | 1.0 | GP噪声方差 |X_train = np.array([[0, 1, 2, 0], [1, 0, 1, 2]])  # 示例数据

| `prior_variance` | float | 1.0 | GP先验方差 |y_train = np.array([0.8, 0.6])

| `variable_types` | Dict | None | 变量类型映射，如{0: "categorical"} |acq_fn.fit(X_train, y_train)

| `coverage_method` | str | "min_distance" | 覆盖度计算方法 |

# 评估候选点

## 数学原理X_candidates = np.array([[0, 0, 0, 0], [1, 1, 1, 1]])

scores = acq_fn(X_candidates)

采集函数结合了两个组件：best_idx = np.argmax(scores)

```

**α(x; D_t) = α_info(x; D_t) + γ · α_cov(x; D_t)**

### 运行完整实验

### 信息增益组件 (α_info)

**V1 基线实验** (推荐):

``````bash

α_info = (1/|J|) Σ_{j∈J} ΔVar[θ_j | x] + λ_t · (1/|I|) Σ_{(j,k)∈I} ΔVar[θ_jk | x]cd test/categorical_experiment

```pixi run python run_categorical_experiment_v1.py

```

- **主效应项**: 平均每个主效应参数的方差减少

- **交互效应项**: 平均每个交互效应参数的方差减少，权重λ_t动态调整**V2 实验** (仅用于研究对比):

```bash

### 动态权重 (λ_t)cd test/categorical_experiment

pixi run python run_categorical_experiment_v2.py

``````

λ_t = λ_min + (λ_max - λ_min) · sigmoid((r_t - τ_1) / τ_2)

```### 对比分析



其中 `r_t` 是主效应相对不确定性：```bash

cd test/categorical_experiment

```pixi run python compare_v1_vs_v2.py

r_t = mean(Var[θ_j | D_t] / Var[θ_j | D_0])```

```

## 配置说明

- **早期阶段** (r_t ≈ 1): λ_t ≈ λ_min，侧重主效应

- **后期阶段** (r_t → 0): λ_t → λ_max，侧重交互效应### V1 关键参数（AEPsych 配置格式）

### 覆盖度组件 (α_cov)```ini

[common]

```parnames = [color, layout, font_size, background]

α_cov(x; D_t) = min_{x'∈D_t} d_Gower(x, x')stimuli_per_trial = 1

```outcome_types = [continuous]

strategy_names = [init_strat, opt_strat]

使用Gower距离衡量到已采样点的最小距离，鼓励探索未覆盖区域。

[opt_strat]

## 项目结构generator = OptimizeAcqfGenerator

model = GPClassificationModel

```acqf = VarianceReductionWithCoverageAcqf

dynamic_eur_acquisition/

├── acquisition_function_v4.py      # V4主实现 ✅[VarianceReductionWithCoverageAcqf]

├── gower_distance.py               # Gower距离计算lambda_main = 1.0               # 主效应权重

├── gp_variance.py                  # GP方差计算lambda_inter = 0.5              # 交互效应初始权重

├── __init__.py                     # 包导入lambda_inter_max = 3.0          # 交互效应最大权重

├── README.md                       # 本文件tau_main = 0.3                  # 主效应阈值

│tau_inter = 0.5                 # 交互效应阈值

├── configs/                        # 配置示例gamma_diversity = 0.5           # 多样性/覆盖度权重

│   ├── config_example.inicoverage_method = min_distance  # 覆盖度计算方法

│   ├── config_template.iniinteraction_terms = (0,1);(1,3);(2,3)  # 关注的交互项

│   └── simulation_config.ini```

│

├── docs/                           # 详细文档### V2 额外参数 (不推荐使用)

│   ├── README.md

│   ├── QUICKSTART.md```ini

│   └── IMPLEMENTATION_SUMMARY.mdpenalty_repeat = 0.01           # 重复惩罚（太弱，效果不佳）

│gamma_binning = 0.4             # 分箱管理（造成局部陷阱）

├── test/                           # 测试和示例n_bins = 5                      # 分箱数量

│   ├── unit_tests/                 # 单元测试beta_ucb = 0.15                 # UCB 探索

│   ├── integration_tests/          # 集成测试gamma_exploration = 0.3         # 探索奖励权重

│   ├── examples/                   # 使用示例gamma_diversity_min = 0.2       # 多样性最小权重

│   └── simulation_project/         # 仿真项目```

│

├── legacy/                         # 历史版本（V1-V3）## 实验结果对比

│   ├── acquisition_function.py     # V1

│   ├── acquisition_function_v2.py  # V2### 性能指标对比表

│   ├── acquisition_function_v3.py  # V3

│   └── hard_exclusion_generator.py| 指标 | V1 (基线) ✅ | V2 (实验) ⚠️ | 变化 |

│|------|-----------|-----------|------|

└── archive/                        # 历史文档和报告| **唯一设计数** | 39/360 (10.8%) | 28/360 (7.8%) | **-28.2%** ❌ |

    ├── docs/                       # 完整历史文档| **平均重复次数** | 2.05x | 2.86x | **+39.5%** ❌ |

    └── configs/                    # 旧配置文件| **高分设计 (≥9.5)** | 8 个 | 4 个 | **-50.0%** ❌ |

```| **平均分数** | 8.72±0.89 | 7.94±0.60 | **-0.78** ❌ |

| **组件数量** | 2 | 4 | +100% |

## 核心文件| **代码行数** | ~500 | ~850 | +70% |

| **参数数量** | 11 | 17 | +55% |

使用V4只需要以下文件：

### 详细分析

1. **acquisition_function_v4.py** - 主采集函数类

2. **gower_distance.py** - 距离计算工具查看完整的实验分析报告：

3. **gp_variance.py** - 方差估计工具- **失败分析**: `test/categorical_experiment/report/V2_FAILURE_ANALYSIS.md`

4. **__init__.py** - 包导入- **完整报告**: `test/categorical_experiment/FINAL_REPORT_V2.md`

- **可视化对比**: `test/categorical_experiment/report/comparison_v1_vs_v2.png`

## 测试

## 关键发现与教训

```bash

# 运行单元测试### ✅ 成功经验 (来自 V1)

cd test/unit_tests

pixi run python -m pytest test_acquisition_v4.py1. **简单即美**: 双组件设计（信息增益 + 覆盖度）足够有效

2. **稳定权重**: 固定的多样性权重（0.5）优于动态调整

# 运行集成测试3. **渐进策略**: 交互效应权重动态增加（0.5→3.0）符合学习过程

cd test/integration_tests4. **最小距离覆盖**: 有效防止局部过采样

pixi run python test_full_workflow.py

### ❌ 失败教训 (来自 V2)

# 运行示例

cd test/examples1. **过度设计**: 添加过多组件增加调参复杂度而无实质收益

pixi run python example_basic_usage.py2. **软惩罚失效**: 0.01 的重复惩罚无法有效阻止重复采样

```3. **分箱陷阱**: 分箱管理导致在中分区域的过度采样

4. **动态权重风险**: 多样性权重从 0.8 降到 0.2 导致后期过度开发

## 性能特点

### 💡 核心见解

- **计算效率**: 批量评估候选点，支持GPU加速

- **内存优化**: 增量更新方差基线，避免重复计算- **问题诊断**: V1 的唯一问题是重复采样，不是设计框架本身

- **可扩展性**: 支持高维问题（测试到20+维度）- **解决方案**: 应使用硬排除（`-inf`），而非添加复杂机制

- **设计哲学**: 针对性修复优于全面重构

## 版本历史- **实验价值**: 负面结果也是宝贵的知识 - "The only real mistake is the one from which we learn nothing."



- **V4** (当前): forward-only设计，变量类型感知，空间规范化 ✅ **推荐使用**## 数学公式

- **V3**: 硬排除 + 组合策略（见`legacy/`）

- **V2**: 增强版软惩罚（见`legacy/`）### V1 采集函数

- **V1**: 基线版本（见`legacy/`）

**α(x; D_t) = α_info(x; D_t) + α_cov(x; D_t)**

详细的版本对比和历史分析见 `archive/docs/README_FULL_HISTORY.md`

其中：

## 相关文档

- **α_info**: 信息增益项

- **快速入门**: `docs/QUICKSTART.md`  - α_info = (1/|J|) Σ_j ΔVar[θ_j] + λ_t(r_t) × (1/|I|) Σ_{j,k} ΔVar[θ_jk]

- **API文档**: `docs/README.md`  - 主效应 + 动态加权的交互效应

- **实现细节**: `docs/IMPLEMENTATION_SUMMARY.md`  

- **完整历史**: `archive/docs/README_FULL_HISTORY.md`- **α_cov**: 空间覆盖项  

  - α_cov = γ × COV(x; D_t)

## 常见问题  - 基于 Gower 距离到现有样本

  - 使用 min_distance 方法

### Q: 如何选择interaction_terms？

- **λ_t(r_t)**: 动态权重函数

A: 基于领域知识选择可能有交互效应的因素对。如果不确定，可以：  - 随着主效应不确定性降低，增加交互效应权重

- 从所有两两组合开始: `[(i,j) for i in range(n) for j in range(i+1, n)]`  - 分段线性函数，依赖相对方差 r_t

- 或先探索主效应，再根据结果添加交互项

### V2 采集函数（不推荐）

### Q: variable_types必须手动指定吗？

**α(x; D_t) = w_info × α_info + w_div × α_div + w_bin × α_bin + w_expl × α_expl**

A: 不需要。V4会自动从AEPsych的ParameterTransforms推断变量类型。只有在直接使用（不通过AEPsych配置）时才需要手动指定。

其中添加了分箱管理和探索奖励，但增加了复杂度且效果不佳。

### Q: 如何调整探索vs开发的平衡？

## 参数说明

A: 调整以下参数：

- `gamma`: 增大提高空间覆盖（探索），减小提高信息增益（开发）### V1 关键参数

- `lambda_min/max`: 控制交互效应探索的动态范围

- `tau_1/tau_2`: 调整从主效应到交互效应的转换时机| 参数 | 描述 | 默认值 | 范围 |

|------|------|--------|------|

### Q: 支持约束优化吗？| `lambda_main` | 主效应权重 | 1.0 | [0, ∞) |

| `lambda_inter` | 交互效应初始权重 | 0.5 | [0, ∞) |

A: V4本身不处理约束，但可以通过：| `lambda_inter_max` | 交互效应最大权重 | 3.0 | [lambda_inter, ∞) |

- 预过滤候选集（在生成candidates时排除不可行点）| `tau_main` | 主效应方差阈值 | 0.3 | [0, 1.0] |

- 使用AEPsych的约束处理机制| `tau_inter` | 交互效应方差阈值 | 0.5 | [0, 1.0] |

- 后处理：将不可行点的分数设为`-inf`| `gamma_diversity` | 覆盖度权重 | 0.5 | [0, ∞) |

| `coverage_method` | 覆盖度计算方法 | 'min_distance' | min/mean/median |

## 引用| `interaction_terms` | 交互项列表 | [] | [(i,j), ...] |



如果在研究中使用此采集函数，请引用：## 未来方向



```### 推荐：V3 最小化改进

Dynamic EUR Acquisition Function for Mixed-Type Variables

AEPsych Extensions, 2025保持 V1 的全部设计，仅添加硬重复排除：

```

```python

## 许可证class ImprovedAcqfV3(VarianceReductionWithCoverageAcqf):

    """V3: V1 + 硬重复排除"""

遵循AEPsych主项目的许可证。    

    def _evaluate_numpy(self, X_candidates):

---        # 使用 V1 的全部逻辑

        scores = super()._evaluate_numpy(X_candidates)

**维护者**: AEPsych开发团队          

**最后更新**: 2025-10-30          # 唯一改变：硬排除已采样设计

**版本**: V4 (2.1.0)        for i, x in enumerate(X_candidates):

            design_key = self._design_to_key(x)
            if design_key in self._sampled_designs:
                scores[i] = -np.inf  # 硬排除，而非软惩罚
        
        return scores
```

**预期改进**:

- 唯一设计数: 39 → 55-65 (+40-65%)
- 高分设计: 8 → 12-16 (+50-100%)
- 平均分数: 8.72 → 9.0-9.3 (+3-7%)

### 可选优化

1. **候选集预过滤**:
   - 80% 从未采样设计 + 20% 已采样设计
   - 减少评估开销

2. **自适应覆盖权重**:
   - 早期较高（探索）→ 后期较低（开发）
   - 但需谨慎，V2 的教训显示动态权重可能适得其反

## 测试

### 运行单元测试

```bash
# V1 快速验证
cd test/categorical_experiment
pixi run python -c "from acquisition_function_v1 import VarianceReductionWithCoverageAcqf; print('✓ V1 导入成功')"

# 完整实验测试
pixi run python run_categorical_experiment_v1.py
```

### 性能基准

基于 80 次试验、360 个设计空间的分类变量实验：

- **V1**: 10.8% 覆盖率，8 个高分设计，平均 8.72 分
- **V2**: 7.8% 覆盖率，4 个高分设计，平均 7.94 分

## 依赖项

- Python 3.8+
- NumPy
- SciPy
- ConfigParser (标准库)
- Matplotlib (用于可视化)
- AEPsych (主框架)

## 版本历史

- **V1.0** (2025-10-30): 基线版本，简单有效 ✅
- **V2.0** (2025-10-30): 实验版本，失败但有价值 ⚠️
- **V3.0** (计划中): 最小化改进，硬排除重复 🎯

## 贡献与反馈

本项目是 AEPsych 扩展的一部分。实验设计、实现与分析完成于 2025年10月30日。

### 相关文档

- **API 文档**: `docs/README.md`
- **快速入门**: `docs/QUICKSTART.md`
- **实现细节**: `docs/IMPLEMENTATION_SUMMARY.md`
- **V2 失败分析**: `test/categorical_experiment/report/V2_FAILURE_ANALYSIS.md`
- **完整实验报告**: `test/categorical_experiment/FINAL_REPORT_V2.md`

## 许可证

遵循主项目 AEPsych 的许可证。

---

**重要提示**:

- ✅ **推荐使用 V1 版本** - 简单、稳定、效果好
- ⚠️ **V2 仅供研究参考** - 失败实验，但提供了宝贵的教训
- 🎯 **V3 开发方向明确** - 最小化改动，硬排除重复

*"The only real mistake is the one from which we learn nothing." - Henry Ford*
