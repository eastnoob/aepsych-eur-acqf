...
def run_simplified_experiment(acqf_class, acqf_name, n_trials=80, n_init=20, seed=42):
    ...
    results = calculate_metrics(
        X_sampled, y_sampled, true_scores, acqf_name, all_designs
    )
...
def print_comparison(results_dict):
    ...
    row = "覆盖率 (%)"
    values = [
        results_dict[name]["space_coverage"]["coverage_rate"] * 100 for name in names
    ]
    print(f"{row:<30} " + " ".join([f"{v:>12.1f}" for v in values]))

    row = "重复率 (%)"
    values = [
        results_dict[name]["space_coverage"]["repeat_rate"] * 100 for name in names
    ]
    print(f"{row:<30} " + " ".join([f"{v:>12.1f}" for v in values]))
    ...
    row = "高分发现 (≥9.5)"
    values = [
        results_dict[name]["quality_discovery"]["high_score_counts"]["score_ge_9.5"]
        for name in names
    ]
    print(f"{row:<30} " + " ".join([f"{v:>12}" for v in values]))
    ...
    for name in names:
        rates = [
            cov["rate"]
            for cov in results_dict[name]["space_coverage"]["level_coverage"].values()
        ]
    ...
    for name in names:
        rates = [
            cov["rate"]
            for cov in results_dict[name]["space_coverage"][
                "interaction_coverage"
            ].values()
        ]
        avg_rate = np.mean(rates) * 100
...
if __name__ == "__main__":
    ...
    results_v3a = run_simplified_experiment(
        HardExclusionAcqf, "V3A-HardExclusion", n_trials=80, n_init=20, seed=42
    )

    results_v3c = run_simplified_experiment(
        CombinedAcqf, "V3C-Combined", n_trials=80, n_init=20, seed=42
    )

    # 整合结果
    results_dict = {"V1": results_v1, "V3A": results_v3a, "V3C": results_v3c}
